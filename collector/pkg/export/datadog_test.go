...
// buildInlineStack creates a format v2 inline stack with the given frame labels.
func buildInlineStack(frames []struct{ label, path string; line uint32 }) []byte {
var buf []byte
...
func makeSampleEvent(label string) *collector.RubySampleEvent {
stackData := buildInlineStack([]struct{ label, path string; line uint32 }{
{label, "/app/test.rb", 10},
})
...
if err := e.Export(ctx, makeSampleEvent("main")); err != nil {
t.Fatalf("Export: %v", err)
}
if err := e.Export(ctx, makeSampleEvent("ActiveRecord::Base#find")); err != nil {
t.Fatalf("Export: %v", err)
}
if err := e.Export(ctx, makeSampleEvent("main")); err != nil {
t.Fatalf("Export: %v", err)
...
_ = e.Export(ctx, makeSampleEvent("ActionController::Base#process"))
